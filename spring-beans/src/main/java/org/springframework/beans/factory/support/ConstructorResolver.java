...
class ConstructorResolver {
	...
	public BeanWrapper instantiateUsingFactoryMethod(
		...
		// <1> 确定工厂对象： 获得 factoryBean、factoryClass、isStatic、factoryBeanName 属性
		Object factoryBean;
		Class<?> factoryClass;
		...
		// <2> 构造参数确认：获得 factoryMethodToUse、argsHolderToUse、argsToUse 属性
		...
		if (explicitArgs != null) {
			...
			// 如果缓存中存在，则解析存储在 BeanDefinition 中的参数
			// 如给定方法的构造参数 A(int, int)，则通过此方法后就会把配置文件中的("1","1")转换为(1,1)
		...
		if (factoryMethodToUse == null || argsToUse == null) {
			...
			// 排序构造函数
			// public 构造函数优先于参数数量降序，非 public 构造函数参数数量降序
