...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		// BeanWrapper 是 Bean 的一个包装，其接口中所定义的功能很简单，包括设置获取被包装的对象、获取被包装 bean 的属性描述器
		BeanWrapper instanceWrapper = null;
		// <1> 单例模式，则从未完成的 FactoryBean 缓存中删除
		if (mbd.isSingleton()) {
		...
		boolean earlySingletonExposure = (mbd.isSingleton()  // 单例模式
				...
				&& isSingletonCurrentlyInCreation(beanName));  // 当前 bean 是否正在被创建
		if (earlySingletonExposure) {
			if (logger.isTraceEnabled()) {
				logger.trace("Eagerly caching bean '" + beanName +
						"' to allow for resolving potential circular references");
			}
			// 提前将创建的 beanFactory 实例加入到 singletonFactories 中
			// 这里是为了后期避免循环依赖
	 ...
	 * @param beanName the name of the bean
	 * @param mbd tBhe bean definition for the bean
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// <2> 如果 FactoryBean 的 factory-method 来创建，支持静态工厂和实例工厂
	...
	protected BeanWrapper obtainFromSupplier(Supplier<?> instanceSupplier, String beanName) {
		...
		// 设置新的 Bean 对象的名字到 currentlyCreatedBean 中
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					// 当属性存在过这个封装 bean 时，将会找到所有匹配的 bean 并将其注入
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
			...
			// 属性值需要转换
				...
				// 转换属性值，例如将引用转换为 IoC 容器中实例化对象引用！！！对属性值的解析！！
				...
				// 属性是可以转换的，且属性原始值是字符串类型，且属性的原始类型不是动态生成的字符串，且属性的原始值不是集合或者数组类型
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {  // 安全方法
	...
	protected void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
		...
		// 首先会检查是否是 InitializingBean，如果是的话，需要调用 afterPropertiesSet()
