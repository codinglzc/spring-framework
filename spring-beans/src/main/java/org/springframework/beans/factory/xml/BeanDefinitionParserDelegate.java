...
public class BeanDefinitionParserDelegate {
	...
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			...
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) { // 标签名为 meta
				...
				// 添加到 BeanMetadataAtrributeAccessor 中
	...
	public void parseConstructorArgElements(Element beanEle, BeanDefinition bd) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			if (isCandidateElement(node) && nodeNameEquals(node, CONSTRUCTOR_ARG_ELEMENT)) { // 标签名为 construct-arg
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		if ((hasRefAttribute && hasValueAttribute) ||  // 1. ref 和 value 都存在
				// 2. ref 和 value 存在其一，并且 sunElement 存在
